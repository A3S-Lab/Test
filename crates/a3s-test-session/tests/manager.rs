...
use a3s_test_core::{
    ...
    RepairHumanAction, RepairHumanActionKind, RepairStatus, ScenarioContext, StepOutput, Surface,
    SurfaceDriver, SurfaceObservation, Target, TestStep, PAGE_CONTEXT_DIFF_PROTOCOL,
};
use a3s_test_session::{
    ActSessionRequest, AgentSessionManager, FinishSessionRequest, RepairTransition,
    SessionFinishStatus, SessionManagerOptions, StartSessionRequest,
};
use async_trait::async_trait;
use serde_json::json;
use tokio::sync::{Barrier, Mutex, Notify};

#[path = "support/manager_repair.rs"]
mod manager_repair;
mod support;
use manager_repair::{
    manager, prepare_verifying_repair, repair_verify_request, test_repair_finding,
    verify_repair_with_policy,
};
...
#[tokio::test]
async fn verification_rejects_a_change_report_that_differs_from_completion() {
    let state = Arc::new(Mutex::new(FakeState {
        repairs: vec![test_repair_finding()],
        inspect_context: Some(ready_page_context(4)),
        ..FakeState::default()
    }));
    let (_temp, manager) = prepare_verifying_repair("change-mismatch", state).await;
    let mut request = repair_verify_request("change-mismatch");
    request.changed_files = vec!["src/Other.tsx".to_string()];

    let error = manager
        .verify_repair(request)
        .await
        .expect_err("verification must use the append-only completion report");
    assert_eq!(error.code(), "test.session.repair_change_mismatch");
    let loop_record = manager
        .inspect_repair_loop("change-mismatch", "finding-1")
        .await
        .expect("inspect unchanged loop");
    assert_eq!(loop_record.status, RepairStatus::Verifying);
    assert_eq!(
        loop_record.change.expect("completion change").changed_files,
        ["src/Checkout.tsx"]
    );
    manager.abort("change-mismatch").await.expect("abort");
}
