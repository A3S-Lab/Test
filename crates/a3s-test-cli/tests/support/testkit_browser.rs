...
fn verify_shortcut_discoverability(command: &impl Fn(&[&str]) -> Output) {
    ...
    let compact_layout = encoded_eval(
        ...
        "(()=>{const shadow=document.querySelector('[data-a3s-testkit-overlay]').shadowRoot;const settings=shadow.querySelector('.a3s-settings-content');const hide=[...shadow.querySelectorAll('button')].find(candidate=>candidate.textContent.trim()==='Hide until tab restart');const bounds=hide.getBoundingClientRect();const settingsBounds=settings.getBoundingClientRect();const viewport=document.documentElement;return JSON.stringify({settingsHeight:settings.clientHeight,settingsBounds:{top:settingsBounds.top,right:settingsBounds.right,bottom:settingsBounds.bottom,left:settingsBounds.left},viewportHeight:viewport.clientHeight,viewportWidth:viewport.clientWidth,viewportBounded:settingsBounds.top>=0&&settingsBounds.right<=viewport.clientWidth&&settingsBounds.bottom<=viewport.clientHeight&&settingsBounds.left>=0,reachable:bounds.top>=settingsBounds.top&&bounds.bottom<=settingsBounds.bottom})})()",
    );
    assert!(
