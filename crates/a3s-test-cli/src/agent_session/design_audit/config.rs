...
use a3s_test_agent::{
    DesignAuditOptions, DesignAuditProviderIdentity, HttpProviderEndpoint, ProvenanceRedactor,
};
...
#[derive(Debug)]
pub(super) struct DesignAuditConfig {
    pub(super) identity: DesignAuditProviderIdentity,
    pub(super) endpoint: HttpProviderEndpoint,
    authorization_env: Option<String>,
    pub(super) max_cost_microusd: u64,
    pub(super) options: DesignAuditOptions,
}

impl DesignAuditConfig {
...
pub(super) async fn read(path: &Path) -> Result<DesignAuditConfig> {
    let metadata = tokio::fs::symlink_metadata(path)
        .await
        .with_context(|| format!("failed to inspect design-audit config {}", path.display()))?;
    if !metadata.file_type().is_file() || metadata.file_type().is_symlink() {
        anyhow::bail!("design-audit config must be a regular non-symbolic-link file");
    }
    if metadata.len() == 0 || metadata.len() > MAX_CONFIG_BYTES {
        anyhow::bail!("design-audit config must contain 1 to {MAX_CONFIG_BYTES} bytes");
    }
    let source = tokio::fs::read_to_string(path)
        .await
        .with_context(|| format!("failed to read design-audit config {}", path.display()))?;
    parse(&source)
}

fn parse(source: &str) -> Result<DesignAuditConfig> {
    let document = a3s_acl::parse(source).context("invalid design-audit ACL")?;
    if document.blocks.len() != 1 || document.blocks[0].name != "design_audit" {
        anyhow::bail!("design-audit config must contain exactly one design_audit block");
    }
    let root = &document.blocks[0];
    if !root.labels.is_empty() {
        anyhow::bail!("design_audit does not accept labels");
    }
    ensure_attributes(
        root,
        &[
    ...
    if root.blocks.len() != 1 || root.blocks[0].name != "provider" {
        anyhow::bail!("design_audit requires exactly one provider block");
    }
    let provider = &root.blocks[0];
    if !provider.labels.is_empty() || !provider.blocks.is_empty() {
        anyhow::bail!("design_audit.provider does not accept labels or nested blocks");
    }
    ensure_attributes(
        provider,
        &["name", "model", "endpoint", "authorization_env"],
        "design_audit.provider",
    )?;
    let authorization_env =
        optional_string(provider, "authorization_env", "design_audit.provider")?;
    if let Some(name) = &authorization_env {
        validate_authorization_env(name)?;
    }
    let timeout_ms = optional_u64(root, "timeout_ms", 30_000, "design_audit")?;
    Ok(DesignAuditConfig {
        identity: DesignAuditProviderIdentity {
            provider: required_string(provider, "name", "design_audit.provider")?.to_string(),
            model: required_string(provider, "model", "design_audit.provider")?.to_string(),
        },
        endpoint: required_string(provider, "endpoint", "design_audit.provider")?
            .parse()
            .map_err(anyhow::Error::new)?,
        authorization_env,
        max_cost_microusd: required_u64(root, "max_cost_microusd", "design_audit")?,
        options: DesignAuditOptions {
            timeout: Duration::from_millis(timeout_ms),
            max_findings: optional_usize(root, "max_findings", 100, "design_audit")?,
            max_summary_bytes: optional_usize(
                root,
                "max_summary_bytes",
                2 * 1_024,
                "design_audit",
            )?,
            max_rationale_bytes: optional_usize(
                root,
                "max_rationale_bytes",
                8 * 1_024,
                "design_audit",
            )?,
            max_recommendation_bytes: optional_usize(
                root,
                "max_recommendation_bytes",
                8 * 1_024,
                "design_audit",
            )?,
            max_page_context_bytes: optional_usize(
                root,
                "max_page_context_bytes",
                8 * 1_024 * 1_024,
                "design_audit",
...
mod tests {
    ...
    #[test]
    fn parses_bounded_design_audit_acl() {
        let config = parse(
            r#"
design_audit {
  max_cost_microusd = 8000
  timeout_ms = 20000
  max_findings = 24
  max_page_context_bytes = 4194304

  provider {
    name = "deployment"
    model = "design-review"
    endpoint = "https://models.example.test/v1/design-audit"
...
"#,
        ...
        assert_eq!(config.options.max_findings, 24);
        assert_eq!(config.options.max_page_context_bytes, 4_194_304);
        assert_eq!(config.max_cost_microusd, 8_000);
    }

    #[test]
    fn rejects_unknown_blocks_and_unsafe_authorization_names() {
        let unknown_block = parse(
            r#"
design_audit {
  max_cost_microusd = 1
  provider {
...
"#,
        ...
        .expect_err("invalid design-audit config");
        assert!(
            unknown_block
                .to_string()
                .contains("exactly one provider block"),
            "{unknown_block:#}"
        );

        let unsafe_authorization = parse(
            r#"
design_audit {
  max_cost_microusd = 1
  provider {
...
"#,
        ...
        .expect_err("unsafe authorization variable");
        assert!(
            unsafe_authorization
                .to_string()
                .contains("must use the A3S_TEST_PROVIDER_AUTHORIZATION_ prefix"),
            "{unsafe_authorization:#}"
        );
    }
}
