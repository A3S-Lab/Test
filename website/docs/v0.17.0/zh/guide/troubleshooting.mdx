...
检查 `A3SReviewOverlay` 的 `enabled`、外层 provider、页面 `<html lang>` 和 Shadow DOM host。Overlay 只有在 live bridge 存在时挂载。它默认是紧凑 Dock，工具、编辑器和问题工作区会按当前状态展开。

键盘操作无响应时，先确认焦点没有停在输入框或编辑器。字母快捷键在可编辑控件中会主动让路。移动端还要检查宿主页面是否用全屏层或更高 z-index 遮住了 Dock。
