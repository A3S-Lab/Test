...
:::warning 发布候选
本页描述 v0.17.0；安装命令仍使用已发布的稳定版 v0.16.2。
:::
...
当前包尚未发布到 npm Registry，所以这里安装的是 GitHub Release 附带的 `.tgz`。npm 仍会把依赖记录为 `@a3s-lab/testkit`。上面的命令安装已发布的 v0.16.2 稳定包；本页新增的画板和页面区域截图会随 v0.17.0 正式发布，不需要额外安装绘图或截图模块。
