...
ACL 编译器保存现有的 `Action::Assert(Expectation::Visible(target))`，并增加由 Runner 负责的隐藏模式。这个策略不会增加 Action 变体；当前协议推进到修订 8，是因为上面的控件状态 expectation。Runner 派发正向可见性探测，再按以下契约分类。
...
编译器保存现有的可见目标等待条件，并增加 `wait_mode = hidden`；在当前修订 8 下，它仍然复用早先的可见性 Action 变体。只要正向探测仍返回可见证据，Runner 就先立即执行一次只读可见性断言，再每 50 ms 探测一次。
