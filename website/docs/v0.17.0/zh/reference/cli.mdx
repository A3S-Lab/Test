...
Action 协议修订 9 把预期文案绑定到唯一目标，并观察稳定定位器产生的完整可见集合。
...
两种平面都排除零几何元素，但都不声称目标处于 viewport 内或没有被其他像素遮挡。当前浏览器 ref 可以标识唯一 `rendered_text` 元素，却不能表示 `visible_count` 集合。GUI 与 TUI 对两种条件都明确关闭失败。

两种条件都支持 `stable_for_ms`。后续文案或数量不匹配变为 `test.assert.unstable`；驱动失败保留原错误码。检入证据覆盖 600/600 个确定性分类、200/200 个持续一致窗口、200/200 个瞬态窗口，以及真实 Chromium CLI 中 7/7 个正向观察、7/7 个负向分类、两个 100 ms 窗口和零私有 runtime 泄漏。
...
ACL 编译器保存现有的 `Action::Assert(Expectation::Visible(target))`，并增加由 Runner 负责的隐藏模式。这个策略不会增加 Action 变体；当前协议已推进到修订 9，是因为上面的类型化 expectation。Runner 派发正向可见性探测，再按以下契约分类。
...
编译器保存现有的可见目标等待条件，并增加 `wait_mode = hidden`；在当前修订 9 下，它仍然复用早先的可见性 Action 变体。只要正向探测仍返回可见证据，Runner 就先立即执行一次只读可见性断言，再每 50 ms 探测一次。
