...
`hidden` is an immediate assertion, not a disappearance wait. Combine it with `stable_for_ms` when the product must remain hidden. A later visible sample then returns `test.assert.unstable` and retains the first hidden observation plus the visible counterexample.
