...
Generate the complete revision-14 JSON Schema with `a3s-test agent schema`. Current action types are grouped below.
...
### Visual-viewport and pointer-hit ACL expectations

Action protocol revision 12 distinguishes a target that intersects the visual viewport from one that pointer hit testing can reach.
...
`in_viewport` requires positive-area intersection. A partial target passes with an `intersection_ratio` between zero and one; a fully offscreen target or boundary-only contact fails. `pointer_reachable` samples a deterministic 3 by 3 grid over the clipped target rectangle and passes when at least one deep hit reaches the target or a composed-tree descendant. It does not prove enabled state, keyboard access, event handling, or product intent.

Both forms accept stable role, text, test ID, label, placeholder, or CSS locators. ACL rejects refs and visual points, while a current Page Context ref may resolve to a stable locator before dispatch. Semantic targets traverse open Shadow DOM and exclude accessibility-hidden ancestry. CSS keeps visually rendered `aria-hidden` targets. Native hit testing makes a pointer-receiving transparent overlay block and makes a `pointer-events: none` overlay pass through.
...
A passing viewport payload carries both rectangles and the independently recomputed ratio. A passing pointer payload adds all nine ordered sample coordinates and booleans plus `sample_count` and `reachable_samples`. GUI and TUI fail closed because their current protocols do not provide equivalent evidence. Checked-in evidence covers 1,000/1,000 Core geometry cases, 2,000/2,000 Web protocol classifications, 200/200 sustained and 200/200 transient windows, and a standalone Chromium matrix with 20 positive assertions and 15 negative or driver-error classifications plus exact cleanup.
...
The ACL compiler stores the existing `Action::Assert(Expectation::Visible(target))` plus a runner-owned hidden mode. This policy adds no action variant; the current protocol is revision 14 because of the typed expectations above. The runner dispatches a positive visibility probe and uses this contract.
...
The compiler stores the existing visible-target wait condition plus `wait_mode = hidden`; under current revision 14 it still reuses the earlier visible-action variant. Runner sends immediate read-only positive visibility assertions and polls every 50 ms only while visible evidence continues.
