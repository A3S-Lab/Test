...
`auto`, follows `<html lang>`: every `zh-*` tag selects Simplified Chinese and
other languages use English. Pass an explicit locale when the review surface
must not follow the host page.
