...
The Test Kit package version advances independently from the A3S Test Release tag, so the two version numbers do not need to match. Continue with the [three-step Web Test Kit setup](./testkit.mdx#three-step-react-setup). The design board and page-region capture documented here belong to the v0.17.0 release candidate. They will ship inside Test Kit with that Release, without a separate drawing library or screenshot plugin.
