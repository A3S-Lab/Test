...
The ACL compiler keeps `Action::Wait(WaitCondition::Visible(target))` and adds runner-owned `WaitMode::Hidden`, so action protocol revision 7 is unchanged. Runner executes an immediate positive visibility assertion, then repeats it every 50 ms while the target remains visible.
