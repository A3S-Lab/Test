...
## A hidden assertion fails or is inconclusive
...
`hidden` means “no visible match now”; it is not a wait for a target to disappear. If the product transition is still in progress, establish a typed readiness condition first. Use `stable_for_ms` only when the requirement is that the target remain hidden after the first successful hidden probe.
