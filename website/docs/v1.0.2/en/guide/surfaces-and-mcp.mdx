...
| External coding agent needing one tool surface | `a3s-test mcp`               | Session layer for registered Web, GUI, and/or TUI drivers | MCP does not replace deterministic `check` and `run`; persistent `agent` CLI stays Web-only |
...
The locked CUA 0.23.2 configuration admits only macOS installed-daemon and embedded-socket profiles. Windows and Linux fail before transport startup. `contract_tested` means the checked-in protocol contract passes; it does not mean this machine has granted Accessibility and Screen Recording permissions.
...
| `rendered_text`      | Single-target accessibility copy: CUA `value`, then `label`, with the same whitespace normalization as Web                                                                       |
...
TUI supports deterministic ACL and MCP host registration via `--tui-executable`
(alone or with Web/GUI). Persistent `a3s-test agent` CLI sessions remain
Web-only; terminal actions are unavailable there.

## MCP as a uniform session interface for external agents

MCP is a stdio projection of the same session application layer, not a second
runner. A host can register Web, GUI, TUI, or any combination via `--web-url`,
reviewed GUI host options, and/or `--tui-executable`. `test_inspect` and
`test_repair_*` are advertised only when a Web surface is registered.
...
a3s-test mcp \
  ...
  --gui-attach-pid 4242 \
  --gui-macos-process-name A3S \
  --gui-profile window-vision
```

Unpackaged macOS binaries with a missing CUA `bundle_id` require
`--gui-macos-process-name` together with `--gui-attach-pid`. Worker inventory
advertises that attach-only process name on `WorkerGuiCapability`; launch
profiles must omit it.
...
| Page Context     | `test_inspect`                                                                               | Web hosts only: read a bounded page, node, component, or region              |
| Repair recovery  | `test_repair_inbox`, `test_repair_inspect`                                                   | Web hosts only: prioritize one active session, then inspect one durable loop |
| Repair pickup    | `test_repair_watch`, `test_repair_claim`                                                     | Web hosts only: pick up human submissions and claim one attempt              |
| Repair lifecycle | `test_repair_progress`, `test_repair_reply`, `test_repair_complete`, `test_repair_verify`    | Web hosts only: report editing, ask for input, and verify                    |
| Repair terminal  | `test_repair_fail`, `test_repair_cancel`                                                     | Web hosts only: append failure or cancellation without erasure               |

`test_schema` returns the current interactive Action JSON Schema and the actually registered surfaces. Runner-owned `verify_contract` is absent from that interactive schema. `test_repair_inbox` is a browserless read of the active owning session and returns a typed reconciliation action for an expired mutation lease; it never replays a stale edit command. See [Human review and repair](./repairs.mdx) for workspace-wide CLI discovery, attempt, lease, and verification rules.
