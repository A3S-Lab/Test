...
| 外部编码 Agent 需要统一工具接口 | `a3s-test mcp`               | 已注册 Web、GUI 和/或 TUI driver 的会话层       | MCP 不替代确定性 `check` 与 `run`；持久 `agent` CLI 仍仅 Web |
...
当前锁定的 CUA 0.23.2 配置只准入 macOS installed-daemon 与 embedded-socket profile。Windows 和 Linux 在 transport 启动前明确失败。`contract_tested` 说明检入协议测试通过，不代表当前机器已经授予 Accessibility 和 Screen Recording 权限。
...
`in_viewport`、viewport coverage、pointer reachability、URL、`rendered_texts`、visible count、boolean state 和 selected values 当前没有等价 CUA 证据，会以 `test.driver.gui.assertion_unsupported` 关闭失败。单目标 `rendered_text` 读取 CUA `value`，否则回退到 `label`。浏览器网络、tab、frame、DOM 集合状态和终端动作也不会被猜测映射到 GUI。
...
TUI 支持确定性 ACL，并可通过 `--tui-executable`（可与 Web/GUI 组合）注册 MCP host。持久 `a3s-test agent` CLI 仍仅 Web；终端动作在那里不可用。

## MCP 给外部 Agent 的统一会话接口

MCP 把同一 session application layer 投影到 stdio。确定性测试仍由原有 runner 执行。host 可通过 `--web-url`、已审核 GUI host 选项和/或 `--tui-executable` 注册 Web、GUI、TUI 或其任意组合。仅当注册了 Web surface 时才广告 `test_inspect` 与 `test_repair_*`。
...
| Repair 恢复     | `test_repair_inbox`、`test_repair_inspect`                                                   | 先排列一个活动会话，再读取一条持久循环    |
...
`test_schema` 返回当前 interactive Action JSON Schema 和实际注册 surface。Runner 专用 `verify_contract` 不会出现在 interactive Schema 中。`test_repair_inbox` 不连接浏览器，只读取当前活动 owner 的 ledger；修改 lease 过期时会返回类型化对账动作，不会重放旧编辑命令。工作区级 CLI 发现、Repair Ledger 的 attempt、lease 和验证规则见[人工评审与自动修复](./repairs.mdx)。
