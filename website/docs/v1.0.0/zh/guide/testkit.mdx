...
该包已通过 GitHub OIDC provenance 发布到官方 npm Registry。固定安装的 0.4.1 版本已包含画板和页面区域截图，不需要额外安装绘图或截图模块。
...
Test Kit 0.4.1 把评审收敛到一个固定侧栏。最短流程只有三步：选择目标、描述修改、保存或发送。“新反馈”“问题”“偏好设置”是同一侧栏里的三个并列视图。标记工具直接展示；选中目标后，编辑器在原位置替换工具；保存后直接进入问题视图。不会再打开贴着目标的编辑器、第二层工具浮层或嵌套弹窗。
