...
| 让人准确指出问题                | 一个右侧 Review Overlay 完成元素、文本、多选、区域、手绘、画板和浏览器页面截图 |
| 从点选节点定位到代码            | 显式组件边界、DOM 归属注册和可选 Source Map v3 生成排序后的源码跨度            |
| 阻止旧目标继续作用于新页面      | 每份快照与明确发送的 finding 都绑定单调页面修订                                |
...
该包已通过 GitHub OIDC provenance 发布到官方 npm Registry。固定安装的 0.5.0 版本已包含画板、页面区域截图、CLI 实时兼容握手和渲染节点源码映射，不需要额外安装绘图或截图模块。
...
Test Kit 0.5.0 把评审收敛到一个固定侧栏。最短流程只有三步：选择目标、描述修改、保存或发送。“新反馈”“问题”“偏好设置”是同一侧栏里的三个并列视图。标记工具直接展示；选中目标后，编辑器在原位置替换工具；保存后直接进入问题视图。不会再打开贴着目标的编辑器、第二层工具浮层或嵌套弹窗。
