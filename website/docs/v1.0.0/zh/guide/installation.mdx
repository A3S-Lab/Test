...
页面点选、框选、截图或草图只需安装 [Web Test Kit](#安装-web-test-kit)，终端探索与 ACL 使用 [CLI 与 Agent Skill](#安装-cli-与-agent-skill)。要把页面问题交给编码 Agent 修复并验证，两者都安装。
...
`@a3s-lab/testkit` 0.6.1 已通过 GitHub OIDC provenance 发布到官方 npm Registry。固定版本可以让安装结果可复现，包管理器也会把完整性信息写入项目 lockfile。
...
Test Kit 的包版本独立于 A3S Test 的 Release 标签，因此两个版本号不必相同。安装完成后，按[三步接入 Web Test Kit](./testkit.mdx#三步完成-react-接入)挂载评审界面。Test Kit 0.6.1 已包含简化评审侧栏、有界 SVG 画板、页面内框选截图、CLI 实时兼容握手、渲染节点源码映射和修订级 Page Context 差异，不需要浏览器扩展、绘图库、截图插件或屏幕共享权限。
