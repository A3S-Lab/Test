...
这些命令已经进入 `main`，会随下一版本发布；已发布的 v1.0.0 二进制尚不包含它们。

| 命令     | 作用                                                         |
| -------- | ------------------------------------------------------------ |
| `init`   | 发现前端工程并写入 `.a3s-test/project.acl`。                 |
| `doctor` | 检查配置、可执行文件、包、Test Kit 与可选 URL 连通性。       |
| `dev`    | 打开一个有界页面评审会话，并持续转交用户明确发送的 finding。 |
...
| `test.spec.viewport_coverage_target_unstable`   | ACL 覆盖率目标是浏览器 ref 或 visual point     |
| `test.spec.viewport_coverage_threshold_trivial` | ACL 阈值会让覆盖率断言恒真                     |
| `test.spec.viewport_coverage_percent_limit`     | ACL 覆盖率百分比大于 100                       |
...
| `test.assert.viewport_coverage_at_least`        | 独立重算的合法覆盖率低于最低阈值               |
| `test.assert.viewport_coverage_at_most`         | 独立重算的合法覆盖率高于最高阈值               |
