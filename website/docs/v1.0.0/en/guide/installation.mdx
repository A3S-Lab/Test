...
`@a3s-lab/testkit` 0.4.2 is published on the official npm Registry with GitHub OIDC provenance. Pinning the version keeps installation reproducible, and the package manager stores its integrity in the project lockfile.
...
The Test Kit package version advances independently from the A3S Test Release tag, so the two version numbers do not need to match. Continue with the [three-step Web Test Kit setup](./testkit.mdx#three-step-react-setup). Test Kit 0.4.2 includes the design board, page-region capture, and the live CLI compatibility handshake without a separate drawing library or screenshot plugin.
