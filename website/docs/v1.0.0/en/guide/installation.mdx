...
| --------------- | ----------------------------------------------- |
| npm             | `npm install --save-dev @a3s-lab/testkit@0.4.1` |
| pnpm            | `pnpm add --save-dev @a3s-lab/testkit@0.4.1`    |
| Yarn            | `yarn add --dev @a3s-lab/testkit@0.4.1`         |
| Bun             | `bun add --dev @a3s-lab/testkit@0.4.1`          |

`@a3s-lab/testkit` 0.4.1 is published on the official npm Registry with GitHub OIDC provenance. Pinning the version keeps installation reproducible, and the package manager stores its integrity in the project lockfile.
...
The Test Kit package version advances independently from the A3S Test Release tag, so the two version numbers do not need to match. Continue with the [three-step Web Test Kit setup](./testkit.mdx#three-step-react-setup). Test Kit 0.4.1 includes the design board and page-region capture without a separate drawing library or screenshot plugin.
