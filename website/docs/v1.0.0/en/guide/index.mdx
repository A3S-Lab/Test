---
title: From intent to a repeatable Web test
description: Ground one coding-agent action in the current browser revision, retain the result as evidence, and preserve the proven path as ACL.
---

# From intent to a repeatable Web test

A3S Test turns a vague instruction into a short, checkable loop. The browser provides current facts. The coding agent chooses one typed action. A3S Test admits that action, records what happened, and lets a stable path become an ACL regression.

## The mental model

```text
observable intent
  -> current browser revision
  -> smallest useful action or difference
  -> owning source when needed
...
Page changes expire old `@eN` and `@cN` refs. `@uN` can never be used in `click`, `fill`, `drag`, or another action. This prevents an agent from silently applying an old decision to a new page state.
