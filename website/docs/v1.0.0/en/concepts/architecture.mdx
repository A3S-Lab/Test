...
| ----------------------------- | -------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Embedded page context         | `packages/testkit`                                 | Post-render context, exact revision deltas, source mapping, bounded UI evidence, and optional human review |
| Surface-independent contracts | `crates/a3s-test-core`                             | Typed actions, targets, observations, policy inputs, evidence records, results, and driver traits         |
| Persistent interaction state  | `crates/a3s-test-session`                          | Session lifecycle, observation-bound refs, action admission, append-only state, and verification bindings |
| Concrete execution            | `crates/a3s-test-driver-web`, GUI, and TUI drivers | Surface perception, action dispatch, evidence capture, and exact runtime cleanup                          |
| User and agent entry points   | `crates/a3s-test-cli` and MCP adapters             | CLI commands, JSON contracts, project loop, repair coordination, and protocol-facing validation           |
| Deterministic regression      | ACL compiler and runner                            | Static admission followed by bounded actions, waits, assertions, evidence, and cleanup                    |

The Core never branches on a backend name. A deployment injects a typed `SurfaceDriver`; the adapter owns only the capabilities it can prove. Unsupported behavior fails closed.
